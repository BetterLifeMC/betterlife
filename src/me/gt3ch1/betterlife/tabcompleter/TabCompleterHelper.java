				...
				case "trail":
					switch (args.length) {
						...
						case 2:
							switch (args[0]) {
								case "set":
									subCommands = (List<String>) (CommandUtils.getMainConfiguration().getCustomConfig().getList("trail.enabledParticles"));
									for (int i = 0; i < subCommands.size(); i++) {
										String particle = subCommands.get(i);
										if (Arrays.stream(args).anyMatch(particle::contains) && player.hasPermission("betterlife.command.trail.particle." + particle.toLowerCase())) {
								...
								case "add":
									...
									for (Particle p : particles) {
										if (!subCommands.contains(p.toString()) && Arrays.stream(args).anyMatch(p.toString()::contains)) {
